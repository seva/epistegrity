...
| File | Purpose |
|---|---|
| `METHODOLOGY.md` | The rules. Session protocol, commit discipline, failure handling, WaLRuS-DATA format, TDD with test authenticity, six-item post-phase audit, artifact taxonomy, Record Economics. Copy verbatim — no project-specific content. |
| `CYCLE.md` | The operating loop. Orient → Decide → Execute → Audit → Repair → Repeat. RAROC step selection over feasible/choosable splits with decomposability, sanction surfacing, filter-before-rank at the terminal bound, marginal-not-level scoring, the conditional commitment record on Decide output with the Orient commitment sweep, and Revision escalation from Repair. Definition of Success (proof = working solution in production demonstrating expected RAROC), autonomy as the default regime with owner-sanctioned exceptions, disposition as a legislative act. Copy verbatim — no project-specific content. |
| `HORIZONS.md` | Horizon doctrine — PRAROC-7 (Progressive RAROC; the P is the process, 7 is the ladder's bound: commitment classes run at PRAROC-n, n set by commitment cost). Refinement ladder, checkable claims and expiring readings, the terminal bound derived once and compiled, filter-before-rank on non-convertible denominators, profile-not-total, marginal allocation, conditional commitments ("Yes, when" activation, threshold ≠ allocation, margin-based sunset, hysteresis, the six-field commitment record), set 5/7 mechanics, error signatures as an audit checklist. Copy verbatim; canonical prose in `docs/praroc-paradigm.md` and `docs/resonance-matrix.md`; compiled constraints in `docs/scope.md`. |
...
| `docs/resonance-matrix.md` | The Conditional Resonance Matrix — Owner-supplied canonical prose behind `HORIZONS.md` Conditional commitments: system-state transitions (False-Latent → State Change → True-Resonance → False-Decay), the "Yes, when" activation protocol, the funnel of reality, engineering for graceful entropy. Verbatim save for one Owner-ruled amendment (see Origin). Copied for reference; never instantiated. |
...
6. Leave `METHODOLOGY.md`, `CYCLE.md`, `HORIZONS.md`, `ROLES.md`, `docs/praroc-paradigm.md`, `docs/resonance-matrix.md`, and the templates verbatim — they are shared protocol, not project-specific content.
...
Pin discipline: a `.epistegrity-version` bump is one commit unit that syncs every verbatim protocol file (`METHODOLOGY.md`, `CYCLE.md`, `HORIZONS.md`, `ROLES.md`, `docs/praroc-paradigm.md`, `docs/resonance-matrix.md`, templates) to the pinned tree, or declares the deviation explicitly. Verify by blob SHA — `git hash-object` on the local file must equal the upstream blob SHA at the pinned ref. A pin that asserts a state the files do not have is drift, not versioning.
...
`CYCLE.md` and `docs/scope-TEMPLATE.md` were extracted back from [`law-and-order`](https://github.com/seva/law-and-order) — the first downstream instantiation — where the operating loop, RAROC step selection, and the production-RAROC definition of proof were developed and exercised. The autonomy doctrine and owner-sanction rule were extracted back in a second round, after live operation exposed a blocked dependency and the dependency was inverted. A third round extracted the claims-vs-evidence audit step, scaffold pin discipline, and the sanction-is-permission interpretation — after an audit found a falsified claim that internal-consistency checks could not see, a pin bump drifted from its own files, and a gap analysis misread sanction as owner labor. A fourth round extracted `ROLES.md` and selection-function v2 (feasible/choosable/scored split, decomposability, sanction surfacing), validated live by an RCA in which the old feasibility filter was found silently excluding high-value directions. A sixth round extracted Record Economics — one home per fact, per-artifact content standards, the digest step, the Lesson-Mechanism rule, and property enumeration in audit step 4 — plus the bootstrap-artifact alignment (consolidation beyond pointer-only as a declared adaptation), after live operation in `law-and-order`: a record-layer RCA showed prose lessons failing to enforce their own claims, and a subsequent Owner ruling cycle struck the instance couplings out of the living doctrine (dated provenance pointers and enforcer file paths removed from constitutional text — enforcers are named by kind; file paths live in task-state and dated records). The fifth round (allocation doctrine, extracted the same day as its legislation) was voided and reverted; that history is load-bearing on both sides — the void grounds the round-ordering rule (extraction follows live validation, never rides the ruling that produced the doctrine), and the allocation set accordingly awaits live mileage in the instance (its first organic register-lifecycle event) before its own round.
...
A seventh round integrated the Conditional Resonance Matrix — Owner-supplied doctrine, preserved at `docs/resonance-matrix.md` (verbatim save for the ruled 7.1 amendment below), following the PRAROC precedent (owner canon in `docs/`, condensed deltas in the constitution). It contributes what the horizon ladder left implicit: the governance interface for a single commitment's lifecycle. "Yes, when" activation makes approval conditional on independently observable evidence; admissibility bounds "never No" so the filter-before-rank exclusions stay excluded; threshold-≠-allocation keeps activation evidence out of the selection function's seat (entitlement-at-threshold signature); the funding predicate splits from lifecycle state so exit capital survives the death of expansion capital; sunset fires on the margin required for orderly transition, not on return decay; and hysteresis keeps threshold noise from cycling deploy/withdraw. The six-field conditional commitment record (Activation, Allocation, Continuation, Invalidation, Exit, Authority) lands on Decide output and is swept each Orient. Ruled OWNER-DIRECTED (issue #9): by score alone the canon-only option led under the round-5 ordering rule; the Owner's legislative act chose full extraction, with live validation deferred to downstream instances and Audit item 6 watching the new signatures. A 7.1 amendment (same day, Owner-conveyed ruling on issue #9) made two touches: the canon's absolute "A proposal is never No" sentence was replaced in place with the admissibility-bounded form — canon and law no longer disagree at the source; the original text lives in git history (blob f1b2b72) — and Decide pinned the default Activation for recoverable scalar-scored steps: forecast RAROC ≥ 1, with irrecoverable-margin steps taking a profile condition instead of a scalar.
